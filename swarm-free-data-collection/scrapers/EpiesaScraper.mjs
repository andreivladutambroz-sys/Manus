/**
 * EpiesaScraper - Real data extraction from epiesa.ro
 * 
 * Scrapes actual vehicle parts, OEM codes, and prices from epiesa.ro
 * Uses FlareSolverr to bypass Cloudflare protection
 * 
 * Data collected:
 * - Vehicle brands and models (with years)
...
export class EpiesaScraper {
  constructor(flaresolverrManager) {
    this.baseUrl = 'https://www.epiesa.ro';
    this.name = 'epiesa.ro';
  ...
  /**
   * Extract brands from catalog page
   */
  async scrapeBrands(sessionId) {
    try {
      console.log(`[Epiesa] Scraping brands from ${this.baseUrl}/catalog`);
      ...
      for (const selector of brandSelectors) {
        ...
        if (options.length > 0) {
          options.each((i, el) => {
            ...
            if (value && value !== '' && text && text !== 'Select brand') {
              brands.push({
                value,
                name: text
              });
      ...
      console.log(`[Epiesa] Found ${uniqueBrands.length} brands`);

      return uniqueBrands;
    } catch (error) {
      console.error('[Epiesa] Brand scraping failed:', error.message);
      throw error;
    }
  }
  ...
  async scrapeModels(brand, brandValue, sessionId) {
    try {
      console.log(`[Epiesa] Scraping models for brand: ${brand}`);

      // First, navigate to catalog to get brand/model form
      const result = await this.manager.scrapeUrl(
        `${this.baseUrl}/catalog?marca=${encodeURIComponent(brandValue)}`,
        sessionId
      );
      ...
      for (const selector of modelSelectors) {
        ...
        if (elements.length > 0) {
          elements.each((i, el) => {
            ...
            if (value && value !== '' && text && text !== 'Select model') {
              models.push({
                name: text,
                value: value,
                brand: brand,
                sourceUrl: `${this.baseUrl}/catalog?marca=${encodeURIComponent(brandValue)}&model=${encodeURIComponent(value)}`
              });
            }
          });
          break;
      ...
      console.log(`[Epiesa] Found ${uniqueModels.length} models for ${brand}`);

      return uniqueModels;
    } catch (error) {
      console.error(`[Epiesa] Model scraping failed for ${brand}:`, error.message);
      return [];
    }
  }
  ...
  async searchParts({ brand, model, category = null }, sessionId) {
    try {
      console.log(`[Epiesa] Searching parts for ${brand} ${model}`);

      // Build search URL
      let searchUrl = `${this.baseUrl}/cautare-piese?marca=${encodeURIComponent(brand)}&model=${encodeURIComponent(model)}`;
      if (category) {
        searchUrl += `&categorie=${encodeURIComponent(category)}`;
      }

      const result = await this.manager.scrapeUrl(searchUrl, sessionId);

      if (!result.success) {
        console.warn(`[Epiesa] Failed to search parts for ${brand} ${model}`);
        return [];
      }
      ...
      for (const selector of partSelectors) {
        ...
        if (items.length > 0) {
          items.each((i, el) => {
            ...
            const nameEl = $item.find('.product-title, h3, .part-name, a[data-part-name]').first();
            const priceEl = $item.find('.price, .part-price, [data-price]').first();
            const codeEl = $item.find('.part-code, .oem-code, [data-oem], code').first();
            const urlEl = $item.find('a[href*="/piesa/"]').first();
      ...
      console.log(`[Epiesa] Found ${parts.length} parts for ${brand} ${model}`);
      return parts;
    } catch (error) {
      console.error(`[Epiesa] Part search failed:`, error.message);
      return [];
    }
  }
  ...
  async scrapeCatalog(sessionId) {
    try {
      console.log('[Epiesa] Starting catalog scrape...');
      ...
      for (const brand of brands.slice(0, 10)) { // Limit to first 10 brands for testing
        try {
          ...
          console.warn(`[Epiesa] Failed to scrape models for ${brand.name}`);
        }
      }

      console.log(`[Epiesa] Catalog scrape complete: ${catalog.brands.length} brands, ${catalog.totalModels} models`);
      return catalog;
    } catch (error) {
      console.error('[Epiesa] Catalog scrape failed:', error.message);
      throw error;
    }
  }
