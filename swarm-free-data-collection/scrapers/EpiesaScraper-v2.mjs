/**
 * EpiesaScraper v2 - IMPROVED Real data extraction from epiesa.ro
 * 
 * Fixes:
...
export class EpiesaScraper {
  constructor(flaresolverrManager) {
    ...
    this.seenParts = new Set(); // Track duplicates
  }
  ...
  isValidOemCode(code) {
    ...
    if (code.length >= 6 && /[A-Z0-9]{6,}/.test(code)) {
      ...
      if (/^[A-Z]{2}\d{3}$/.test(code)) return false; // OX123, BP001
      if (/^[A-Z]{2}-[A-Z]-\d{3}$/.test(code)) return false; // WP-F-001
      return true;
    }
  ...
  /**
   * Extract brands from catalog page - IMPROVED
   */
  async scrapeBrands(sessionId) {
    try {
      console.log(`[Epiesa v2] Scraping brands from ${this.baseUrl}/catalog`);
      ...
      console.log(`[Epiesa v2] Found ${uniqueBrands.length} brands`);

      return uniqueBrands;
    } catch (error) {
      console.error('[Epiesa v2] Brand scraping failed:', error.message);
      throw error;
    }
  }
  ...
  async scrapeModels(brand, brandValue, sessionId) {
    try {
      console.log(`[Epiesa v2] Scraping models for brand: ${brand}`);
          ...
          elements.each((i, el) => {
            ...
            if (val && val !== '' && txt && txt.length > 0 &&
              ...
              models.push({
                ...
                value: val,
                brand: brand,
                sourceUrl: `${this.baseUrl}/catalog?marca=${encodeURIComponent(brandValue)}&model=${encodeURIComponent(val)}`
              });
            }
          });
      ...
      console.log(`[Epiesa v2] Found ${uniqueModels.length} models for ${brand}`);

      return uniqueModels;
    } catch (error) {
      console.error(`[Epiesa v2] Model scraping failed for ${brand}:`, error.message);
      return [];
    }
  }
  ...
  async searchParts({ brand, model, category = null }, sessionId) {
    try {
      ...
      if (!result.success) {
        console.warn(`[Epiesa v2] Failed to search parts for ${brand} ${model}`);
        return [];
      }
      ...
      console.log(`[Epiesa v2] Found ${parts.length} unique parts for ${brand} ${model}`);
      return parts;
    } catch (error) {
      console.error(`[Epiesa v2] Part search failed:`, error.message);
      return [];
    }
  }
