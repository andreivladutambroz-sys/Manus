...
async function importJSONL() {
  ...
  console.log('📊 Starting optimized JSONL import...');
...
async function insertBatch(conn, batch) {
  ...
  const values = batch
    .map(
      (r) =>
        `('${r.recordKey}', '${r.make}', '${r.model}', ${r.year}, '${r.engine}', '${r.engineCode}', '${r.code}', '${r.system}', '${r.description}', '${escapeSql(r.symptoms)}', '${escapeSql(r.procedures)}', '${escapeSql(r.tools)}', '${escapeSql(r.torque)}', ${r.confidence}, '${escapeSql(r.sourceUrl)}', '${escapeSql(r.rawJson)}')`
    )
    .join(',');

  const sql = `
    INSERT INTO knowledgeBase_raw 
    (record_key, make, model, year, engine, engine_code, error_code, system, description, symptoms_json, repair_procedures_json, tools_json, torque_json, confidence, source_url, raw_json)
    VALUES ${values}
